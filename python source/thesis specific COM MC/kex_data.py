...
GATE_KEYS = ['0', '1']
DATA_KEYS = [pkey+" "+gkey for pkey in PHANTOM_KEYS for gkey in GATE_KEYS ]
#        ['cylinder 0', 'cylinder 1', 'cannula 0', 'cannula 1']
RECON_DCR = [1548426888, 1548427023, 1548425897, 1548426033] #from windows batch
MY_NORMS = r"C:/Users/petct/Desktop/Hugo/Code/PythonMEX/norm_effect/my_norms"
...
def get_listmode():
    limo, _ = listmode()
    return dict(zip(DATA_KEYS, limo))
...
def get_sinograms():
    sinos, _ = sinograms()
    return dict(zip(DATA_KEYS, sinos))

def get_my_norm_paths():
    paths = []
    with os.scandir(MY_NORMS) as it:
        for entry in it:
            if entry.is_file() and entry.name.endswith('.s'):
                paths.append(MY_NORMS+"/"+entry.name)
    return paths

def get_my_norms():
    shape = ( 621,168,400) #tofbin dimension omitted
    dtype= np.float32
    paths = get_my_norm_paths()
    return [ff.get_data(path, shape, dtype) for path in paths]

def get_prepaths():
    lm_paths, _, _  = paths(both=True)
    trim_paths = [path[:-2] for path in  lm_paths]
    return trim_paths

def get_sino_prepaths():
    return [p +"-sino" for p in get_prepaths()]

def get_sino_paths():
    return [p+"-0.s" for p in get_sino_prepaths()]

def get_sino_hdrs():
    return [p + ".hdr" for p in get_sino_paths()]

def get_sino_mainhdrs():
    return [p + ".mhdr" for p in get_sino_prepaths()]
...
def get_keyed_normfactors():
    norms, paths = get_norm_factors()
    return dict(zip(DATA_KEYS, norms))
...
def get_recon():
    ...
    return dict(zip(DATA_KEYS, recon))

def get_v_data():
    dicom_paths = paths(both = True)[-1]
    v_data = [ff.get_v_data(path[:-len("-DICOM")]) for path in dicom_paths]
    return dict(zip(DATA_KEYS, v_data))


V_END = "_000_000.v"
def get_v_path(mhdr):
    return mhdr.replace(".mhdr", V_END)

def get_v_mhdr(v_path):
    return v_path.replace(V_END, ".mhdr")
